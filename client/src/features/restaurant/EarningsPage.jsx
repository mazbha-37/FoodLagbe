...
import { useGetMyRestaurantQuery, useGetEarningsQuery } from './restaurantApi';
...
export default function EarningsPage() {
  ...
  const { data: restData } = useGetMyRestaurantQuery();
  const restaurantId = restData?.restaurant?._id;
  ...
  const recentOrders = earningsData?.recentOrders || [];
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              ...
              <StatCard
                icon={Percent}
                label="Commission"
                value={formatCurrency(stats.commissionDeducted || 0)}
                sub={`${stats.commissionRate || 10}% platform fee`}
                color="#7E808C"
              />
              <StatCard
                icon={DollarSign}
            ...
            {chartData.length > 0 && (
              <div className="bg-white rounded-[8px] border border-[#E0E0E0] p-5">
                <h3 className="font-semibold text-[#1C1C1C] mb-4">Revenue Over Time</h3>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={chartData} margin={{ top: 0, right: 0, left: 0, bottom: 0 }}>
                    ...
                    <Bar dataKey="revenue" fill="#E23744" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            <div className="bg-white rounded-[8px] border border-[#E0E0E0]">
              ...
              {recentOrders.length === 0 ? (
                <div className="px-5 py-10 text-center text-sm text-[#7E808C]">No orders in this period</div>
              ) : (
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="border-b border-[#E0E0E0]">
                        ...
                        <th className="text-left px-5 py-3 text-xs text-[#7E808C] font-medium">Customer</th>
                        <th className="text-left px-5 py-3 text-xs text-[#7E808C] font-medium">Date</th>
                        <th className="text-right px-5 py-3 text-xs text-[#7E808C] font-medium">Amount</th>
                        <th className="text-right px-5 py-3 text-xs text-[#7E808C] font-medium">Status</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#E0E0E0]">
                      {recentOrders.map((order) => (
                        <tr key={order._id} className="hover:bg-[#F9F9F9]">
                          <td className="px-5 py-3 font-medium text-[#1C1C1C]">#{order._id?.slice(-6).toUpperCase()}</td>
                          <td className="px-5 py-3 text-[#7E808C]">{order.customerId?.name || 'N/A'}</td>
                          <td className="px-5 py-3 text-[#7E808C]">{new Date(order.createdAt).toLocaleDateString()}</td>
                          <td className="px-5 py-3 text-right font-semibold text-[#1C1C1C]">{formatCurrency(order.total || 0)}</td>
